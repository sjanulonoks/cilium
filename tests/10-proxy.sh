...
RETURN=$(docker exec -i client bash -c "curl -s --output /dev/stderr -w '%{http_code}' --connect-timeout 10 -XGET -H 'X-Test: true' http://$SERVER_IP4:80/headertest")
if [[ "${RETURN//$'\n'}" != "404" ]]; then
	abort "GET /headertest, unexpected return"
fi
