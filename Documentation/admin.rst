...
The easiest and preferred way of deploying Cilium in an existing Kubernetes
cluster is to use a `DaemonSets <https://kubernetes.io/docs/admin/daemons/>`_.
This will automatically deploy and run a ``cilium/cilium`` container image in
the form of a pod on each Kubernetes worker node.

Mounting the BPF FS (Optional)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The only deployment step that can not be performed using a DaemonSet is the
mounting of the BPF filesystem in the host mount namespace. This step is
optional but allows the Cilium agent to pin BPF resources to a persistent
filesystem which marks the resources persistent across agent restarts. If the
BPF filesystem is not mounted in the host filesystem, then all BPF resources
created in the namespace of the pod will be released when the pod is stopped.
This would result in network connectivity loss of all locally managed pods.
...
In order to mount the BPF filesystem, run the following command in the host
mount namespace:
